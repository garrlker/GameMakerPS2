...
// #############################################################################################
/// Function:<summary>
///             Sets the blending color of the tile with the given id. Only available in the Standard Edition!
///          </summary>
///
/// In:		 <param name="id"></param>
...
function tile_layer_find(_depth,_x,_y) 
{
    for(var index in g_RunRoom.m_Tiles )
    {
        ...
        if((_depth == pTile.depth) && (_x >= pTile.x) && (_x < (pTile.x + pTile.w)) && (_y >= pTile.y) && (_y < (pTile.y + pTile.h)) )
		{
			r_lasttile = pTile.id;
			return pTile.id;
		}
