...
function yyPath() 
{
	...
	this.closed = 0; 			// whether the path must be closed
	this.precision = 0; 			// number of subdivisions for smooth paths
	this.Clear();
}
...
yyPath.prototype.AddInternalPoint = function (_xx, _yy, _ss) {
	...
	{
		this.intcount++;
		if (this.intcount > this.intpoints.length)
		{
			//MemoryManager::SetLength( (void**)&i_intpoints.arr, (i_intcount+10)*sizeof(*i_intpoints.arr), __FILE__, __LINE__);
			//i_intpoints.length = i_intcount+10;
		}
		var pWith = new yyIntPoint();
		this.intpoints[intcount - 1] = pWith;
		pWith.x = _xx;
		pWith.y = _yy;
		pWith.speed = _ss;
	}
...
yyPath.prototype.ChangePoint = function (_ind, _xx, _yy, _ss) {
	...
	this.points[ind].x = xx;
	this.points[ind].y = yy;
	this.points[ind].speed = ss;
...
yyPath.prototype.InsertPoint = function (_ind, _xx, _yy, _ss) {
	...
	pPoint = new yyIntPoint(_xx, _yy, _ss);
