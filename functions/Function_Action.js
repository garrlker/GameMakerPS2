...
function action_set_cursor()                    { ErrorFunction("action_set_cursor()"); }
...
function action_if_question()                   { action_unknown(); }
...
{ 
    ...
    if ( true == Argument_Relative )
    {
        _inst.setspeed ( _inst.speed + size );
    }
    else
    {
        _inst.setspeed ( size );
    ...
    switch(d)
    {
        case 0: _inst.setdirection ( 225 ); break;
        case 1: _inst.setdirection ( 270 ); break;
        case 2: _inst.setdirection ( 315 ); break;
        case 3: _inst.setdirection ( 180 ); break;
	    case 4: { _inst.setdirection ( 0 ); _inst.setspeed ( 0 ); break; }
        case 5: _inst.setdirection ( 0 ); break;
        case 6: _inst.setdirection ( 135 ); break;
        case 7: _inst.setdirection ( 90 ); break;
        case 8: _inst.setdirection ( 45 ); break;
    }    
}
...
function action_wrap(_inst, _wrap)
{
    ...
    _inst.wrap(_inst, (wrap == 0) || (wrap == 2) ? true : false, (wrap == 1) || (wrap == 2) ? true : false);
}
...
{
    // _useall doesn't actually have an effect in the VC_Runner so we should be able to get away with ignoring it here too
    move_contact_solid(_inst, _dir, _maxdist) 
}
...
{    
	...
	inst.setspeed( _speed );
    inst.setdirection(_dir);
	inst.PerformEvent(EVENT_CREATE, 0, inst, inst);
	inst.created = true;
}
...
function action_sprite_color(_col)
{
    draw_set_color(_col);
}
...
function action_draw_life(_inst, _x, _y, _life_str)
{
    ...
    if ((lifeStr != null) || (lifeStr == undefined))
    {
        lifeStr = "";
    }
...
function action_set_health(_inst, _health)                    
{
    ...
    {
	    event_perform(_inst, EVENT_OTHER, EVENT_OTHER_NOHEALTH);
    }
}
...
{   
    if ( true != Background_Exists(Round(arg[0].val)) )
    {
        debug(error_a0005);
        return;
    }
...
function action_draw_health(_inst, _x, _y, _x2, _y2, _col, _col2)
{
    ...
    if (Argument_Relative )
    {
        draw_healthbar(_x+_inst.x,_y+_inst.y, _x2+_inst.x, _y2+_inst.y, g_pBuiltIn.health, col1, col2, col3, col4, 0, (_col >= 0.5)?true:false, true);
    }
    else
    {
        draw_healthbar(_x,_y, _x2, _y2, g_pBuiltIn.health, col1, col2, col3, col4, 0, (_col >= 0.5)?true:false, true);
    }
}
...
function action_current_room(_transition)
{
    room_restart();
}
...
function action_linear_step(_inst, _x, _y, _stepsize, _checkall)
{
    if (Argument_Relative)
    {
        mp_linear_step(_inst, _x + _inst.x, _y + _inst.y, _stepsize, OBJECT_ALL, (_checkall >= 0.5) ? true : false);
    }
    else
    {
        mp_linear_step(_inst, _x, _y, _stepsize, OBJECT_ALL, (_checkall >= 0.5) ? true : false);
    }
}
...
function action_potential_step(_inst, _x, _y, _stepsize, _checkall)
{
    if (Argument_Relative)
    {
        mp_potential_step(_inst, _x + _inst.x, _y + _inst.y, _stepsize, OBJECT_ALL, (_checkall >= 0.5) ? true : false);
    }
    else
    {
        mp_potential_step(_inst, _x, _y, _stepsize, OBJECT_ALL, (_checkall >= 0.5) ? true : false);
    }
}

function action_inherited(_inst)
{    
...
function action_kill_position(_inst, _x, _y)
{
    if (Argument_Relative)
    {
        position_destroy(_x + _inst.x, _y + _inst.y);
    }
    else
    {
...
{
    action_timeline_set(_inst, _index, _pos);    
}

function action_timeline_set(_inst, _index, _pos)
{
...
{
    highscore_set_background(Round(_background));
    highscore_set_border((Round(_border) == 1) ? true : false);
    highscore_set_colors(Round(_col1), Round(_col2));
    highscore_set_font(_fontname, _fontsize, _fontstyle);
    
    highscore_show(g_pBuiltIn.score);    
    g_pIOManager.Clear();
}
