...
{
	var pPath = g_pPathManager.Paths[_ind];
	if (!pPath) return 0;
...
function path_get_speed(_ind,_pos) 
{
	...
	return pPath.SpeedPosition(_pos);
}
...
{
	var pPathDst = g_pPathManager.Paths[_ind]; 	// dont test _ind, as it will simply read "null/undefined" here.
	if (!pPathDst) return;

	var pPathSrc = g_pPathManager.Paths[_path]; 	// dont test _ind, as it will simply read "null/undefined" here.
	if (!pPathSrc) return;
...
{
	_inst.Assign_Path(_path, _speed, 1, 0, _absolute, _endaction);
}
...
///			 </returns>
// #############################################################################################
function draw_path( _id, _xoff, _yoff, _absolute )
{
	var pPath  = g_pPathManager.Paths[_id];
	if (!pPath) return;
...
///			 </returns>
// #############################################################################################
function path_scale( _id, _xscale, _yscale )
{
	var pPath  = g_pPathManager.Paths[_id];
	if (!pPath) return;
	
	pPath.Scale(_xscale,_yscale);
}
...
function path_change_point(_id, _ind,_xx,_yy,_speed) {
	...
	pPath.ChangePoint(_ind, _xx, _yy, _speed);
}
...
function path_append(_ind, _path) {
	var pDest = g_pPathManager.Paths[_ind]; 	
	if (!pDest) return;
	var pSrc = g_pPathManager.Paths[_path]; 	
	if (!pSrc) return;
...
function path_insert_point(_ind, _n, _x, _y, _speed) {
	...
	pPath.InsertPoint(_n, _x, _y, _speed);
}
