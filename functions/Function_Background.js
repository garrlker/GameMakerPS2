...
///          </summary>
///
/// In:		 <param name="_ind">index of background</param>
/// Out:	 <returns>
///				true for yes, false for no.
///			 </returns>
// #############################################################################################
function background_exists(_ind)
{
	if (g_pBackgroundManager.Get(_ind) === null) return false;
    return true;
}
...
///			 </returns>
// #############################################################################################
function background_get_name(_ind)
{
	var pBack = g_pBackgroundManager.Get(_ind);
    if( !pBack) return "";
    return pBack.pName;
}
function background_name(_ind){ return background_get_name(_ind); }
...
///			 </returns>
// #############################################################################################
function background_get_width(_ind)
{
	var pBack = g_pBackgroundManager.Get(_ind);
	if (!pBack) return 0;
	pBack = g_pBackgroundManager.GetImage(pBack.index);
    return pBack.TPEntry.w;
}
...
///			 </returns>
// #############################################################################################
function background_get_height(_ind)
{
	var pBack = g_pBackgroundManager.Get(_ind);
    if( !pBack) return 0;
    pBack = g_pBackgroundManager.GetImage(pBack.index);
    return pBack.TPEntry.h;
}
...
function draw_background(_back,_x,_y) 
{
    ...
    Graphics_TextureDrawSimple( pImage.TPEntry, _x, _y );
}
...
function draw_background_stretched(_back, _x, _y, _w, _h) {
	var pImage = g_pBackgroundManager.GetImage(_back);
	Graphics_DrawStretchedExt(pImage.TPEntry, _x, _y, _w, _h, 0xffffff, 1.0);
}
...
function draw_background_tiled(_back,_x,_y) 
{
    ...
    Graphics_TextureDrawTiled( pImage.TPEntry, _x, _y, true, true );
}
...
{
	var pImage = g_pBackgroundManager.GetImage(_back);
	if( pImage!=null && pImage!=undefined )
	{
		Graphics_DrawPart(pImage.TPEntry, _left, _top, _width, _height, _x, _y);
	}
}
...
function draw_background_ext(_back,_x,_y,_xscale,_yscale,_rot,_color,_alpha) 
{
	...
	if( pImage!=null && pImage!=undefined )
	{
	    Graphics_TextureDraw_RELEASE( pImage.TPEntry,  0,0,  _x,_y, _xscale, _yscale, _rot, ConvertGMColour(_color), _alpha);	
	}
}
...
function draw_background_general(_back,_left,_top,_width,_height,_x,_y,_xscale,_yscale,_rot,_c1,_c2,_c3,_c4,_alpha) {
	...
	if (pBack != null && pBack != undefined)
	{
		...
		Graphics_DrawGeneral(pBack.TPEntry, _left, _top, _width, _height, _x, _y, _xscale, _yscale, _rot, _c1, _c2, _c3, _c4, _alpha);
	}
}
...
function background_set_alpha_from_background(_ind, _back) 
{
	...
	if (pDest != null && pDest != undefined && pSrc != null && pSrc != undefined)
	{
		if (!pDest.copy)
		{
			...
			pDest.TPEntry.texture = g_Textures[pBack.TPEntry.tp];
			pDest.TPEntry.x = 0;
			pDest.TPEntry.y = 0;
			pDest.copy = true;
		}
