...
///			</returns>
// #############################################################################################
function place_empty(_pInst,_x,_y) 
{
	var xx,yy, Result,pInstance;
...
{
  _pInst.sethspeed(_x-_pInst.x);
  _pInst.setvspeed(_y-_pInst.y);
  _pInst.setspeed(_speed);
}
...
function Command_Bounce( _pInst, _advanced, _useall)
{
    ...
    if( _advanced )
    {
		...
		if (didbounce == true){
			_pInst.setdirection(180 + (ldir + rdir) - dir);
		}
	}
    else
    {
...
{
	Command_Bounce( _pInst, _adv, false );
}
...
{
	Command_Bounce( _pInst, _adv, true );
}
...
{        
	...
	dx = Math.cos(_dir*Math.PI/180);
    dy = -Math.sin(_dir*Math.PI/180);
...
{
	move_contact(_pInst, _dir, _maxdist, false);
}
...
{
    move_contact(_inst, _dir, _maxdist, true);
}
...
{   
    move_outside(_inst, _dir, _maxdist, false); 
}
...
{
    move_outside(_inst, _dir, _maxdist, true);
}
...
function distance_to_object(_inst,_obj) 
{
    ...
    var i = 0;
    var object_id = Instance_SearchLoop( _inst, Math.floor(_obj), false, dist, 
        function (_pInstance) 
		{
			var xd,yd;
...
function position_empty(_inst,_x,_y) 
{
	...
	var found = Instance_SearchLoop(_inst, OBJECT_ALL, false, false,
        function (_pInstance) {
        	return _pInstance.Collision_Point(_x, _y, true);
        }
    );
	return !found;
}
...
function position_meeting(_pInst,_x,_y,_obj) 
{
    ...
    if( _obj == OBJECT_SELF ) _obj = _inst.id;
	...
	for(var inst in pool)
	{
		...
		if (pInstance.Collision_Point(_x, _y, true))
		{ 
            Result = true;
			break;
		}
